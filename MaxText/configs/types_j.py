...
def _load_and_merge_yamls(
    filepath: str, config_dir: Optional[str] = None
) -> Dict[str, Any]:
    ...
    if "base_config" in cfg and cfg["base_config"]:
        base_config_path = os.path.join(config_dir, cfg.pop("base_config"))
        base_cfg = _load_and_merge_yamls(base_config_path, config_dir)
...
def _update_with_model_vars(config_dict: dict, maxtext_path: str) -> dict:
    ...
    if model_name and model_name != "default":
        ...
        if model_path:
            ...
            model_config = _load_and_merge_yamls(
                model_path, os.path.dirname(model_path)
            )
            # Main config (config_dict) overrides the model's base config
            return {**model_config, **config_dict}

    return config_dict


def initialize(argv: List[str], **kwargs) -> MaxTextConfig:
    """
    Initializes the configuration by parsing YAML files, command-line arguments, and keyword arguments.

    The precedence for configuration values is:
    1. Keyword arguments to this function (highest).
    2. Command-line arguments in `key=value` format.
    3. The user-provided YAML file.
    4. The `model_name` specific YAML file.
    5. The `base_config` YAML file(s).
    6. Default values defined in the Pydantic model (lowest).

    Args:
        argv: A list of command-line arguments, where one argument must be the
              path to the primary YAML config file.
        **kwargs: Keyword arguments that override any other configuration values.

    Returns:
        A validated Pydantic `MaxTextConfig` object.

    Raises:
        ValueError: If no YAML file is specified in `argv`.
        ValidationError: If the final merged configuration is invalid.
    """
    # Find and load base YAML configuration from argv
    config_file = next(
        (arg for arg in argv[1:] if arg.endswith((".yml", ".yaml")) and "=" not in arg),
        None,
    )
    ...
    merged_config = _update_with_model_vars(merged_config, maxtext_path)

    # Merge overrides from both CLI strings and keyword arguments.
    #   Keyword arguments (`kwargs`) take final precedence.
    string_overrides = _parse_cli_key_value_pairs(argv[1:])
    final_overrides = {**string_overrides, **kwargs}
    merged_config.update(final_overrides)

    # Validate and instantiate the Pydantic model
    try:
        config_instance = MaxTextConfig.model_validate(merged_config)
