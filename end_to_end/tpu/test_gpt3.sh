...
python3 -m maxtext.convert_gpt3_ckpt_from_paxml --paxml-ckpt-path=${PAXML_CKPT_PATH} --maxtext-model-name=gpt3-52k --run-name=${RUN_NAME} --base-output-directory=${OUTPUT_PATH}

# Run gpt3-52k with the converted ckpt
python3 -m maxtext.train src/maxtext/configs/base.yml run_name=${RUN_NAME} model_name=gpt3-52k\
