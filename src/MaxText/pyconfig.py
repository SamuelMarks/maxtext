...
def initialize(argv: list[str], **kwargs) -> HyperParameters: 
    ...
    model_cfg = {} 
    if model_name != "default" and not temp_cfg.get("override_model_config"):
        ...
        if os.path.exists(model_config_path):
            model_cfg = omegaconf.OmegaConf.load(model_config_path)
        else:
            logger.warning(f"Model config for '{model_name}' not found at {model_config_path}")
    ...
    for k in list(raw_keys_dict.keys()): 
      ...
      if env_key in os.environ: 
        if k in cli_keys or k in kwargs_keys: 
          raise ValueError(f"Key '{k}' is overridden by both CLI/kwargs and environment variable '{env_key}'. This is not allowed.") 
        
        new_proposal = os.environ.get(env_key) 
        original_value = raw_keys_dict.get(k) 
        parser = None
        if isinstance(original_value, bool): parser = _yaml_types_to_parser[bool] 
        elif isinstance(original_value, (str, int, float)): parser = type(original_value) 
        
        if parser is None: 
            raise TypeError(f"Type {type(original_value)} for key '{k}' not supported for ENV override.") 

        try: raw_keys_dict[k] = parser(new_proposal) 
        except (ValueError, KeyError) as e: raise ValueError(f"Couldn't parse value from ENV '{new_proposal}' for key '{k}'") from e
    ...
    if config.log_config: 
        for k, v in sorted(config.get_keys().items()): 
            if k != "hf_access_token": 
                logger.info(f"Config param {k}: {v}") 
