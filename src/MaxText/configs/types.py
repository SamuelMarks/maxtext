...
class HardwareAndMesh(BaseModel): 
  ...
  context_parallel_strategy: str = Field("all_gather", description="Strategy for context parallelism ('all_gather' or 'ring').") 
...
class DatasetGeneral(BaseModel): 
  ...
  max_segments_per_seq: int = Field(32, description="Maximum number of segments that can be packed into a single sequence.") 
...
class RLEvaluation(BaseModel): 
  ...
  generation_configs: dict[str, Any] = Field(default_factory=dict) 
...
class Reward(BaseModel): 
  ...
  reward_exact_format_match: float = 3.0
  reward_white_space_format_match: float = 1.5
  reward_partial_format_match: float = 0.5
  reward_ratio_guess_to_answer_high: float = 0.5
  reward_ratio_guess_to_answer_low: float = 0.25
  penalty_incorrect_format: float = -0.5
  penalty_incorrect_answer: float = -1.0

class SpecialTokens(BaseModel): 
  """Special tokens used for formatting prompts and responses in RL.""" 

  reasoning_start_token: str = Field("<reasoning>") 
  reasoning_end_token: str = Field("</reasoning>") 
  solution_start_token: str = Field("<answer>") 
  solution_end_token: str = Field("</answer>") 
...
class MaxTextConfig( 
  ...
  def load_model_specific_defaults(cls, values: dict[str, Any]) -> dict[str, Any]: 
      """No-op. `pyconfig` handles model-specific config loading.""" 
      return values
  ...
  def set_derived_and_validate_values(self) -> "MaxTextConfig": 
    ...
    This logic is ported from the legacy pyconfig_deprecated.py system. 
    """ 
    ...
    if tokenizer_path and not os.path.isabs(tokenizer_path) and not tokenizer_path.startswith("gs://"): 
      self.tokenizer_path = os.path.join(MAXTEXT_ASSETS_ROOT, self.tokenizer_path) 
    ...
    try: 
      ...
      print("Warning: JAX device system not available for config validation. Assuming 1 device.", file=sys.stderr) 
    ...
    object.__setattr__(self, "num_slices", max_utils.get_num_slices(raw_keys_for_num_slices)) 
    ...
    def calculate_global_batch_sizes(per_device_batch_size, expansion_factor, grad_accum_steps): 
    ...
    ( 
        ...
        self.per_device_batch_size, self.expansion_factor_real_data, self.gradient_accumulation_steps
    ) 
    ...
    ) = calculate_global_batch_sizes(self.eval_per_device_batch_size, self.expansion_factor_real_data, 1) 

    # G. RUN ALL CROSS-FIELD VALIDATIONS
    ...
    # Emergency/Multi-tier Checkpointing
    ...
    if self.quantization in ("fp8", "nanoo_fp8") and self.gradient_accumulation_steps > 1: 
      raise ValueError("FP8 quantization is not compatible with gradient accumulation.") 

    # MoE Validations
    if self.num_experts > 1: 
      is_fully_moe = self.interleave_moe_layer_step == 1 and self.first_num_dense_layers == 0
      if is_fully_moe and self.base_mlp_dim != self.base_moe_mlp_dim: 
        # This is a specific workaround for qwen3-moe models loaded from HF
        # where base_mlp_dim might not be explicitly set in the config, 
        # but for a fully MoE model it should be equal to base_moe_mlp_dim. 
        if self.decoder_block == DecoderBlockType.QWEN3_MOE: 
            self.base_mlp_dim = self.base_moe_mlp_dim
            # Also need to update derived mlp_dim
            _, _, mlp_dim_scale, _ = get_individual_scales(self.global_parameter_scale) 
            self.mlp_dim = (2**mlp_dim_scale) * self.base_mlp_dim
        else: 
            raise ValueError("For a fully MoE model, base_mlp_dim must equal base_moe_mlp_dim.") 
      if self.decoder_block == "gpt_oss" and not self.sparse_matmul and self.capacity_factor != -1: 
    ...
    if (self.per_device_batch_size * self.max_target_length) % self.num_vocab_tiling != 0: 
    ...
    if self.context_parallel_size is not None and self.context_parallel_size > 1 and self.context_parallel_strategy.lower() == "ring": 
    ...
    if self.decoder_block == DecoderBlockType.LLAMA4: 
      ...
      if self.base_num_decoder_layers % self.interleave_moe_layer_step != 0: 
        raise ValueError( 
            f"The number of decoder layers ({self.base_num_decoder_layers}) must be divisible by interleave moe layer step ({self.interleave_moe_layer_step})" 
        ) 

    if self.decoder_block == DecoderBlockType.QWEN3_NEXT: 
      if self.sparse_matmul: 
        logger.warning( 
            "For Qwen3-Next, sparse_matmul must be False for now. The dense path has been verified against reference. Forcing to False." 
        ) 
    ...
    # Data Loading Validations
    if "tokenizer_llama3.tiktoken" in self.tokenizer_path and self.tokenizer_type != TokenizerType.TIKTOKEN: 
    ...
    # H. FINAL TYPE CONVERSIONS AND DERIVED LISTS
    ...
    # These final conversions must happen inside the pydantic model for validation to pass
    # for list/tuple types. When HyperParameters wrapper runs `_lists_to_tuples`, it does so
    # on the dumped dictionary, not on the pydantic model itself. 
    ...
    # If constant_bound_config is a string, convert it to a list of floats
    ...
    if isinstance(constant_bound_config, str): 
      if constant_bound_config: 
        ...
        self.constant_bound_config = []  # Handle empty string case
